...
# ;c:\jigsaw\classes\jigsaw.jar;c:\jigsaw\classes\xp.jar;c:\jigsaw\classes\sax.jar;c:\jigsaw\classes\servlet.jar;c:\jigsaw\classes\Tidy.jar;c:\jigsaw\classes\tagsoup-1.2.jar;C:\Jigsaw\classes\commons-lang-2.4.jar;C:\Jigsaw\classes\commons-collections-3.2.1.jar 
...
class CSSPlugin(Plugin):
	...
	def install(self):
		...
		for fname, url in download_and_extract.items():
			path = os.path.join(self.installation, fname)
	...
	def validate(self, fname):
		...
		jigsaw = os.path.join(self.installation, "jigsaw")
		validator = os.path.join(self.installation, "css-validator.jar")

		classpath += validator + os.pathsep
		classpath += os.path.join(self.installation, "velocity-1.5.jar") + os.pathsep
		classpath += os.path.join(self.installation, "xerces.jar") + os.pathsep
		...
		return self.run_command_line("java -classpath %s -jar %s %s" % (classpath, validator, fname))